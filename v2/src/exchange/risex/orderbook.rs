...
struct StoredLevel {
    level: BestLevel,
    checksum_price: String,
    checksum_size: String,
}
    ...
    pub fn apply(
        ...
        if let Some(expected) = delta.checksum {
            ...
            if actual != expected {
                ...
                book.asks.clear();
                anyhow::bail!(
                    "RiseX checksum mismatch for market {market_id}: expected {expected}, actual {actual}; fresh snapshot required"
                );
            }
        }
    ...
    for level in levels {
        if level.level.size.value() == 0 {
            ...
            side.insert(
                ...
                StoredLevel {
                    level: level.level,
                    checksum_price: level.checksum_price,
                    checksum_size: level.checksum_size,
                },
            );
        }
    }
    ...
    fn checksum(&self) -> u32 {
        ...
        loop {
            ...
            if let Some(level) = bid {
                parts.push(level.checksum_price.as_str());
                parts.push(level.checksum_size.as_str());
            }
...
mod tests {
    ...
    #[test]
    fn rejects_checksum_mismatch_and_requires_a_fresh_snapshot() {
        let snapshot = r#"{
            "channel":"orderbook",
            "type":"snapshot",
            "market_id":"1",
        ...
        let mut books = RisexBooks::default();
        let error = books.apply(snapshot, 123, Some(instrument())).unwrap_err();
        assert!(error.to_string().contains("fresh snapshot required"));
    ...
    #[test]
    fn validates_mainnet_decimal_levels_against_the_wei_checksum_domain() {
        let snapshot = r#"{
            "channel":"orderbook",
            "type":"snapshot",
            "market_id":"1",
