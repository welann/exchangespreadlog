...
pub struct RiseLevelDelta {
    pub level: BestLevel,
    pub checksum_price: String,
    pub checksum_size: String,
}
...
fn parse_orderbook(envelope: Envelope, data: Value) -> Result<OrderbookDelta> {
    ...
    let encoding = level_encoding(&envelope, &payload.bids, &payload.asks);
    ...
    Ok(OrderbookDelta {
        ...
        bids: parse_ws_levels(payload.bids, encoding)?,
        asks: parse_ws_levels(payload.asks, encoding)?,
...
fn level_encoding(envelope: &Envelope, bids: &[WsLevel], asks: &[WsLevel]) -> LevelEncoding {
    // The documented feed uses 18-decimal wei integers. The current mainnet
    // gateway emits decimal levels and identifies its messages with
    // `worker_timestamp`; a decimal point is an additional unambiguous signal.
    let decimal_level = bids
        .iter()
        .chain(asks)
        .any(|level| level.price.contains('.') || level.quantity.contains('.'));
    if envelope.worker_timestamp.is_some() || decimal_level {
        LevelEncoding::Decimal
    } else {
        LevelEncoding::Wei18
    }
}

fn parse_ws_levels(levels: Vec<WsLevel>, encoding: LevelEncoding) -> Result<Vec<RiseLevelDelta>> {
    levels
        .into_iter()
        .map(|level| {
            let price = fixed_from_wire(&level.price, encoding)
                .map_err(|err| anyhow!("invalid RiseX price: {err}"))?;
            let size = fixed_from_wire(&level.quantity, encoding)
                .map_err(|err| anyhow!("invalid RiseX quantity: {err}"))?;
            let checksum_price = checksum_wire_value(&level.price, price, encoding)?;
            let checksum_size = checksum_wire_value(&level.quantity, size, encoding)?;
            Ok(RiseLevelDelta {
...
fn fixed_from_wire(input: &str, encoding: LevelEncoding) -> Result<Fixed> {
    let input = input.trim();
    match encoding {
        LevelEncoding::Decimal => Fixed::from_str(input),
        LevelEncoding::Wei18 => Fixed::from_str(&decimal_from_wei(input)),
    }
}

fn checksum_wire_value(input: &str, value: Fixed, encoding: LevelEncoding) -> Result<String> {
    if matches!(encoding, LevelEncoding::Wei18) {
        return Ok(input.trim().to_string());
    }
    let missing_scale = 18_u32
        .checked_sub(value.scale())
        .ok_or_else(|| anyhow!("RiseX decimal value has more than 18 fractional digits"))?;
    let factor = 10_i128
        .checked_pow(missing_scale)
        .ok_or_else(|| anyhow!("RiseX checksum scale overflow"))?;
    value
        .value()
        .checked_mul(factor)
        .map(|value| value.to_string())
        .ok_or_else(|| anyhow!("RiseX checksum value overflow"))
}
...
mod tests {
    ...
    #[test]
    fn parses_sub_one_tenth_quantity_as_wei_not_as_a_decimal_integer() {
        let raw = r#"{
            "channel":"orderbook",
            "type":"snapshot",
            "market_id":"1",
            "data":{
                "market_id":1,
                "bids":[{"price":"1000000000000000000","quantity":"10000000000000000"}],
                "asks":[]
            }
        }"#;
        ...
        assert_eq!(delta.bids[0].level.price.to_string(), "1");
        assert_eq!(delta.bids[0].level.size.to_string(), "0.01");
    }
    ...
    fn parses_live_mainnet_orderbook_snapshot_with_decimal_levels() {
        ...
        assert_eq!(delta.bids[0].checksum_price, "59571800000000000000000");
        assert_eq!(delta.bids[0].checksum_size, "121659000000000000");
    }
