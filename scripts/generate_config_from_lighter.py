...
def fetch_risex_instruments(lighter_markets: list[LighterMarket]) -> list[Instrument]:
    ...
    for market in markets:
        ...
        if not raw_symbol or market_id is None:
            continue
        active = (
            bool(market.get("available", True))
            and bool(market.get("visible", True))
            and bool(market.get("active", True))
            and bool(config.get("unlocked", True))
        )
        if not active:
            continue
...
def render_config(
    ...
    generator_script: str = "scripts/generate_config_from_lighter.py",
) -> str:
    lines = [
        ...
        "enabled = true",
        toml_key_value("daily_at_utc", daily_at_utc),
        toml_key_value("generator_script", generator_script),
        f"market_limit = {market_limit}",
        "",
        "[storage]",
        'mode = "clickhouse"',
        'jsonl_dir = "data/bbo"',
    ...
    for venue in venues:
        lines.extend(
            [
                ...
                f"enabled = {'true' if venue.instruments else 'false'}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--refresh-at-utc",
        default="00:05",
        help="UTC HH:MM time retained in the generated refresh settings.",
    )
    parser.add_argument(
        "--generator-script",
        default="scripts/generate_config_from_lighter.py",
        help="Generator path retained in the generated refresh settings.",
    )
